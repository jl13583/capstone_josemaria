...
unknowns2 = sorted(
    [s for s in set(beta.values()) if s not in sol0 and isinstance(s, sp.Symbol)],
    key=str
)
...
if polys:
    # --- Recompute unknowns and params from the actual free symbols in polys ---
    all_free = sorted(
        set().union(*[f.free_symbols for f in polys]),
        key=lambda s: s.name
    )
    ...
    print(f"  Total polynomials: {len(polys)}")
    ...
    if use_char3:
        ...
        try:
            if structure_version == 1:
                ...
                polys_for_gb = list(polys)
                polys_mod3 = preprocess_polys_in_char_p(polys_for_gb, 3, gens_all, invert_pairs=None) # reducing polys mod 3
            ...
            for i, p in enumerate(polys_mod3):
                ...
                if den != 1:
                    print("WARNING: denominator {den} in equation {i}: {p}")
                    break
        ...
        try:
            G = groebner(polys, *unknowns2, order='lex', domain=sp.EX, method='buchberger')
            print("Groebner basis size:", len(G))
        except Exception as e:
            print("Groebner(QQ) failed:", e)

#----------------------------------------------------------------------------------------------------        
# --- Exporting reduced_polys (after linear reduction) to a Python file for SageMath verification ---
#----------------------------------------------------------------------------------------------------

# ============================================================
# Corrected export for Sage — Structure 1 and 2
# Recovers u,v expressions hidden inside Type A taus
# ============================================================

# --- Step 1: Build tau → u,v expression map from sol_tuple (Structure 1 only) ---
tau_to_uv_expr = {}

if structure_version == 1:
    for symb, expr in zip(unknowns2, sol_tuple):
        # If linsolve assigned a single tau to represent symb,
        # and symb is NOT a plain b_ symbol (i.e. it's a u,v expression),
        # then that tau = symb (the u,v expression)
        if (expr in tau_syms                                         # expr is a bare tau symbol
            and not (isinstance(symb, sp.Symbol)
            and str(symb).startswith('b_'))):               # symb is NOT a b_ symbol
            tau_to_uv_expr[expr] = symb                             # e.g. tau0 → (u-v)**2

    print("=== Type A taus recovered (tau → u,v expression) ===")
    for tau, expr in sorted(tau_to_uv_expr.items(), key=lambda x: str(x[0])):
        print(f"  {tau}  →  {expr}")

# --- Step 2: Substitute Type A taus back into polynomials ---
if structure_version == 1 and tau_to_uv_expr:
    polys_substituted = []
    for p in reduced_polys_mod3:
        p_sub = sp.expand(p.subs(tau_to_uv_expr))
        # Re-reduce mod 3 after substitution
        try:
            p_sub = sp.Poly(p_sub, *gens_all, modulus=3).as_expr()
        except Exception:
            p_sub = sp.expand(p_sub)
        if sp.expand(p_sub) != 0:
            polys_substituted.append(p_sub)
else:
    # Structure 2: no tau absorption happened, use as-is
    polys_substituted = list(reduced_polys_mod3)

print(f"\nPolynomials after substituting Type A taus: {len(polys_substituted)}")

# --- Step 3: Collect actual free symbols after substitution ---
all_free_syms = sorted(
    {s for p in polys_substituted for s in p.free_symbols},
    key=lambda s: s.name
)
...
print(f"\nParameters ({len(param_syms)}):  {[str(s) for s in param_syms]}")
print(f"Unknowns  ({len(unknown_syms)}): {[str(s) for s in unknown_syms]}")
print(f"Total ring variables for Sage: {len(var_names_export)}")

# --- Step 4: Export ---
export_path = f'/Users/josemarialoza/capstone/polys_for_sage_lss{structure_version}.py'
with open(export_path, 'w') as f:
    f.write(f"# Auto-generated for SageMath — Structure {structure_version}\n")
    f.write(f"# Type A taus have been substituted back to u,v expressions.\n")
